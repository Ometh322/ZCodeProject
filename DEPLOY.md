# Деплой Poker Lounge на Yandex Cloud
...
```bash
git clone https://github.com/твой-пользователь/poker-club.git
cd poker-club
```
...
tar --exclude='node_modules' --exclude='dist' --exclude='*.db' -czf poker.tar.gz -C C:\Users\ostro\ZCodeProject .

# Передать на ВМ (замени IP)
scp poker.tar.gz ubuntu@158.160.x.x:~/
```
**На ВМ:**
```bash
mkdir ~/poker-club && cd ~/poker-club
tar -xzf ~/poker.tar.gz
rm ~/poker.tar.gz
cd ~/poker-club
```
...
В папке проекта на ВМ (`~/poker-club`):
...
Полезные команды (выполнять в папке `~/poker-club` на ВМ):
